...
class GCNIIdense_model(torch.nn.Module):
    ...
    def forward(self,data):
        ...
        #x, edge_index, edge_weight = data.x, edge_index, data.edge_attr
        _hidden = []
...
def main():
    ...
    #sparse_tensor = torch.sparse_coo_tensor(data.edge_index, edge_weight, size=(data.num_nodes, data.num_nodes))
    #adj_2 = torch.sparse.mm(sparse_tensor, sparse_tensor)
    #adj_2.coalesce().indices().shape
    #edge_index = adj_2.coalesce().indices().to(device)
    data = data.to(device)
